...
instruction = f'Instruction: given a dialog context, you need to continue a conversation with an individual.'
...
def predict(model):
    ...
    channel = 2
    shift_pct = 0.4
    AudioSegment.from_wav("recording1.wav").export("evaluate.mp3", format="mp3")
    audio_file = "evaluate.mp3"
...
def Dementia():
    ...
    def inner():
      ...
      #       print(generated_text)
              ...
              response = generate(instruction, knowledge, conversation)
              print(response)
              engine.say(response)
              engine.runAndWait()
              print("Predition: "+ str(predict(eval_model)))
              yield response+'\n'+"Predition: "+ str(predict(eval_model))+'\n'
